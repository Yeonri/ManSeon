...
import com.mansun.be.domain.point.dto.response.allPoint.*;
import com.mansun.be.domain.point.dto.response.forecastOnePointResDto;
import com.mansun.be.domain.point.entity.FishingPoint;
import com.mansun.be.domain.point.entity.QFishingPoint;
import com.mansun.be.domain.point.entity.dataSet.*;
import com.mansun.be.domain.point.fishingpoint.repository.*;
...
//        Wave를 불러오기 위해 필요한 Marinezone Lzone(PK) 코드 리스트
        ...
        LocalDateTime endDateTime = LocalDate.now().plusDays(3).atTime(LocalTime.MIN);
...
//        날씨 조회하는 QueryDSL 문
        ...
        NumberTemplate<Integer> hourExpr = Expressions.numberTemplate(
                Integer.class,
                "HOUR({0})", qWeather.weatherTime
        );
        // 한 번에 3일치 예보 조회
